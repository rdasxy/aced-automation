...
################################################################################################################################################################
#Screen presented to the user to approve/deny new student registrations.
class NewStudentRegistrations(webapp.RequestHandler):
    #Show screen listing all unapproved students.
    #Also, if 'accept' or 'deny' are available, process those requests.
    ...
    def get(self):
        try:    
            ...
            if currentlyLoggedInUser:
                            ...
                            if enrollment.campus != "No campus chosen":
                                ...
                                if enrollmentDuplicatesQuery.count() > 1:
                                    self.response.out.write("This student is already signed up for the same campus for this semester. Cannot approve enrollment."+
                                                            "<br/>To delete this enrollment, please 'deny' the student and then delete him/her from the \"Denied Students List\" ")
                                    return
                                #Approve enrollment.
                                ...
                                return
                #If a deny request was found, deny the student.
                elif self.request.get('deny'):
                    enrollmentsQuery = db.GqlQuery("SELECT * FROM Enrollment WHERE studentID = '" + self.request.get('deny') + "'")
                    ...
                    enrollment.put()
                    self.response.out.write("Student " + self.request.get('accept') + " put in denied list.")
                #Display list of all new students that haven't been accepted yet.
                ...
                currentSemesterEnrollments = db.GqlQuery("SELECT * FROM Enrollment WHERE semesterCode='" + semesterCode + "' AND approved=False AND denied=False")
                studentIDandName = {}
                ...
                self.response.headers['Content-Type'] = 'text/html'
                path = os.path.join(os.path.dirname(__file__), 'newStudentRegistrations.html')
